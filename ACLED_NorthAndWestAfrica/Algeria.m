...
clear all
close all
posW = csvread('Algeria_posdirected.csv');
negW = csvread('Algeria_negdirected.csv');
%name=importdata('Algeria_names.txt');
name=importdata('Algeria_shortnames.txt');
...
posW(14,:)=posW(14,:)+posW(18,:);
posW=posW([1:17,19:end],:);
posW(:,14)=posW(:,14)+posW(:,18);
posW=posW(:,[1:17,19:end]);

negW(14,:)=negW(14,:)+negW(18,:);
negW=negW([1:17,19:end],:);
negW(:,14)=negW(:,14)+negW(:,18);
negW=negW(:,[1:17,19:end]);

name=name([1:17,19:end],1);
...
myaxis=[-0.009 0.011 -0.0104 0.0106  -0.0105 0.0105];
figure;
hold on;
axis off;
for i = 1:n
